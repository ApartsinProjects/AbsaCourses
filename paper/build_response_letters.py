...
NFAT = {
    ...
    "points": [
        ("1. Human validation of the audit on the synthetic corpus", "ok/wip",
         ...
         "New experiment underway. The audit is already validated three independent ways: a behavioral perturbation control it passes by construction, cross-provider judge agreement, and direct agreement with human labels on two annotated corpora (Cohen's kappa 0.56, and 0.62 with a fully independent model family). To extend this evidence to the synthetic text itself, we have prepared a human annotation of a stratified sample of the synthetic corpus (300 reviews, three raters, per-aspect presence and sentiment, blind to the declared labels) and report aspect and sentiment agreement against both the declared labels and the audit. This measures the audit's agreement on synthetic text directly.",
         "Section 5.7; Section 6.1; annotation instrument <code>paper/human_annotation/</code>."),
        ("2. Match filtering subsets to isolate faithfulness", "ok/wip",
         ...
         "New experiment underway. We add a covariate-matched filtering comparison that holds aspect, polarity, aspect-count, length, and style fixed across the retained and control subsets and scores both on a common gold-present aspect set, so the filtering gain is attributable to faithfulness rather than to differing prediction masks. The size-matched result already reported (retaining the top 50% cuts sentiment error at half the training cost, 7 of 8 seeds, replicated across architectures) motivates this stricter matched design.",
         "Section 5.7."),
        ...
        ("7. Rule out generator-specific split effects", "ok",
         ...
         "Clarified, with a new control. The learnable signal is not a single generator's artifact: regenerating and auditing with three independent model families reproduces the label fidelity, and a held-out-generator transfer check evaluates a detector trained on one generator's data on other generators' data. An overlap-generalization analysis further separates aspect-composition effects from domain shift.",
         "Appendix A.19; Appendix A.24."),
    ],
}
...
H7LN = {
    ...
    "points": [
        ("1. Statistical realism and full-schema re-annotation", "ok/wip",
         ...
         "Reframed, with analysis and a new experiment. The realism that matters for a labeled training benchmark is functional: a model trained on the synthetic corpus recovers real aspect and sentiment signal on independent external corpora. Judged sentence by sentence, synthetic sentences are near-indistinguishable from real ones (the judge is only 60% accurate, near the 50% floor, versus 93% on whole reviews), and a sentence-level distributional check places the synthetic corpus close to real on the units that carry the supervision. Comprehensive human re-annotation is addressed by the synthetic-corpus annotation study prepared for point-N1.",
         "Section 6.1; Appendix A.23."),
        ...
        ("4. The 841 token-capped rows", "ok",
         ...
         "Done. Regenerating exactly those rows at a higher cap eliminates the truncation and restores full-corpus length adherence. We further show the truncated rows are not systematically biased: their aspect distribution is not skewed (chi-square p=0.23, Cramer's V=0.034), their polarity distribution is negligibly different (V=0.020), and their audit faithfulness is statistically identical to the complete corpus (0.573 versus 0.577, p=0.76). They differ only in the mechanical ways (fewer aspects, shorter text), so excluding them does not change the benchmark.",
         "Appendix A.14."),
        ("5. Bottom-quartile rows and training value", "ok",
         ...
         "Clarified. This is the signal the filtering recipe exploits: the negative-control bottom quartile collapses on sentiment error across both architectures and both transfer targets (0 of 8 seeds recovering), which is why retaining the top 50% by audit score reduces error at half the training cost. The audit score discriminates informative from uninformative rows at both ends of the distribution.",
         "Section 5.7."),
...
DWED = {
    ...
    "points": [
        ...
        ("4. Strengthen the transfer-limits statement", "ok",
         ...
         "Done. Section 6.1 states that only 9 of 20 aspects are externally validated, that synthetic-only training recovers about 60% of a real-trained model, that the full schema is not yet externally validated, and that high-stakes use requires human-in-the-loop review.",
         "Section 6.1."),
